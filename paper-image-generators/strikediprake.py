...
p.add_mesh(pv.Sphere(radius=0.3, center=xy_plane.center), color="red")  # Center point
p.add_mesh(xy_plane, color="lightgreen", opacity=0.5, show_edges=True)
# p.add_floor()
northing_arrow = pv.Arrow(
    start=xy_plane.center,
    direction=(0, 1, 0),
    scale=8,
    tip_length=0.20,
    tip_radius=0.03,
...
strike_arrow = pv.Arrow(
    start=xy_plane.center,
    direction=strike_vec,
    scale=5,
    tip_length=0.25,
    tip_radius=0.05,
    shaft_radius=0.02,
...
trans_plane.rotate_vector(vector=n_vec, angle=rake, point=(0.0, 0, 0))
p.add_mesh(trans_plane, color="lightblue", opacity=0.5, show_edges=True)
