...
class _GeostoryGenerator(_abc.ABC):
    ...
    def __init__(self, model_bounds=None, model_resolution=None, config=None, **kwargs):
        ...
        **kwargs : dict, optional
        ...
        """
        self.model_bounds = model_bounds or ((-3840, 3840), (-3840, 3840), (-1920, 1920))
...
class MarkovGeostoryGenerator(_GeostoryGenerator):
    ...
    def _build_markov_sequence(self) -> List[str]:
        ...
        sequence = self.mc.simulate(steps=self._MAX_STEPS, 
...
class MarkovMatrixParser:
    ...
    MARKOV_DIRECTORY = os.path.join(os.path.dirname(os.path.abspath(genmodule.__file__)), 'markov_matrix')
    ...
    def __init__(self, path = None):
        ...
        self.states, self.transition_matrix = self._load_transition_matrix_from_csv(self.path)
        # Run a check on the matrix that it is valid markov matrix
        self._validate_transition_matrix()
    
    def _get_default_path(self):
        return os.path.join(MarkovMatrixParser.MARKOV_DIRECTORY, 'default_markov_matrix.csv')
    ...
    def _build_event_dictionary(self):
        ...
        try:
            ...
            event_dictionary = {
                name: cls for name, cls in events.__dict__.items()
    ...
    def _load_transition_matrix_from_csv(self, path):
        ...
        with open(path, mode='r') as file:
            ...
            # Extract the first row to get the state names, ignoring the first column
            header_row = next(reader)
            # Skip the comment line if it exists
            if header[0].startswith('#'):
                header_row = next(reader)  # Read the actual header
            ...
            for state in states:
                if state not in self.event_dictionary:
                    raise ValueError(f"Invalid state name '{state}' in the CSV. Must be one of: {list(self.event_dictionary.keys())}")
            ...
            for i, row in enumerate(reader):
                ...
                if row_label not in self.event_dictionary:
                    raise ValueError(f"Invalid row label '{row_label}' in the CSV. Must be one of: {list(self.event_dictionary.keys())}")

                # Ensure that the row label matches the corresponding state
                if row_label != states[i]:
                    raise ValueError(f"Row label '{row_label}' does not match the corresponding state '{states[i]}' in the header.")
...
class YAMLGeostoryGenerator(_GeostoryGenerator):
        ...
        for category, entries in self.data["vocab"].items():
            for entry in entries:
                ...
                if not hasattr(geowords, class_name):
                    raise ValueError(f"The class {class_name} does not exist in the geowords module")
    ...
    def _sample_sentences(self, n_samples: int = 1) -> List[List[geowords.GeoWord]]:
        ...
        filled_sentences = [self.word_selector.fill_grammar_with_words(structure)
                            for structure in sentence_structures]
        return filled_sentences

    def _sentence_to_history(self, sentence: List[geowords.GeoWord]) -> List[GeoProcess]:
